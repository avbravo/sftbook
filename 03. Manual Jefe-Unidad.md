...
Este manual se divide en 4 etapas con explicación es específicas
* Etapa I
  * Crear un proyecto,un plan de trabajo, explicar cómo se usa el tablero, administrar tarjeta
...
El tablero es un elemento que es visto por los colaboradores de manera simultáneamente.Esto provoca que en ciertas condiciones un colaborador edité una tarjeta al mismo tiempo en que otro colaborador intenta editarla. 
El sistema evitá qué dos usuarios actualicen al mismo tiempo la tarjeta. Se procesa la primera petición y la siguiente es rechazada, por lo tanto el segundo colaborador debe actualizar su tablero presionando el botón actualizar, ahora tendrá su registro sincronizado con los últimos cambios de la base de datos y puede ingresar sus cambios e intentar actualizar la tarjeta. Este proceso se repite hasta que se garantice que la actualización no entra e conflicto con otros procesos sobre el mismo documento.

Aquí colocar el mensaje que está actualizado por otro usuario poner la foto.
